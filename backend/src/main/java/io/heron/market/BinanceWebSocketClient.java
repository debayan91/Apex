 ...
 * - Configurable via heron.websocket.enabled (set to false in tests)
 */
@Component
@Slf4j
...
public class BinanceWebSocketClient implements ApplicationListener<ApplicationReadyEvent> {
    ...
    @Value("${heron.websocket.enabled:true}")
    private boolean enabled;

    @Override
    public void onApplicationEvent(ApplicationReadyEvent event) {
        if (!enabled) {
            log.info("[WEBSOCKET] WebSocket client is disabled via heron.websocket.enabled=false");
            return;
        }
        startStreaming();
    }
